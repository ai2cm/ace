...
if [[ "$EXPERIMENT_DIR" =~ coupled ]]; then
    ...
    FME_MODULE_INFERENCE="fme.coupled.inference"
else
    FME_MODULE_VALIDATE="fme.ace.validate_config"
    FME_MODULE_INFERENCE="fme.ace.inference"
fi
...
while read TRAIN_EXPER; do
    ...
    RESTART_DATASET=$(echo "$TRAIN_EXPER" | cut -d"|" -f15)
    ...
    if [[ -n $EXISTING_RESULTS_ATMOS_DATASET ]]; then
        # this is for the coupled StandaloneComponentCheckpointsConfig
        CHECKPOINT_DATASET_ARGS+=(
    ...
    GROUP="$JOB_GROUP"
    N_GPUS=1
    FME_MODULE="$FME_MODULE_INFERENCE"
    ...
    if [[ "$DRY_RUN" == "true" ]]; then
        ...
        echo
        echo "Launching ${EXPERIMENT_DIR} inference job:"
    ...
    python -m $FME_MODULE_VALIDATE --config_type inference "$CONFIG_PATH_REL" --override "${OVERRIDE_ARGV[@]}"
    ...
    if [[ "$DRY_RUN" != "true" ]]; then
        ...
        gantry run \
            ...
            -- python -I -m $FME_MODULE_INFERENCE "$CONFIG_PATH_REL" --override "${OVERRIDE_ARGV[@]}"
        echo
    fi
done <"$INPUT_PATH"
