...
for YEAR in 2015 1995; do
    ...
    CONFIG="${SCRIPT_PATH}/ace-inference-4deg-v2-no-residual-no-co2-10year-${YEAR}-raw-ec-uah.yaml"
    ...
    gantry run --name "$JOB_NAME" --task-name "$JOB_NAME" \
        --description "Energy-corrector inference (uah=ERA5-matched) im4ecamc" \
