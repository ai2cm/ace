#!/bin/bash
# Launch energy-corrector 10-year re-runs with decade-matched unaccounted heating.
set -e
JOB_NAME_BASE="ace-4deg-v2-no-co2-2ki61qpw-10year"
JOB_GROUP="2ki61qpw-10year-energy-corrector-uah"
...
for YEAR in 2015 1995; do
    ...
    CONFIG="${SCRIPT_PATH}/ace-inference-4deg-v2-no-co2-10year-${YEAR}-raw-ec-uah.yaml"
    ...
    gantry run --name "$JOB_NAME" --task-name "$JOB_NAME" \
        --description "Energy-corrector inference (uah=ERA5-matched) 2ki61qpw" \
