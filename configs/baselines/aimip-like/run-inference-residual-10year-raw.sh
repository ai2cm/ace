#!/bin/bash
# Launch the standalone raw-output 10-year re-runs of the no-CO2, residual-prediction
# 4deg v2 model (wandb 2ki61qpw): the 2015-2024 out-of-sample decade and its
# 1995-2004 in-sample twin. Run from configs/baselines/aimip-like/.
set -e

JOB_NAME_BASE="ace-4deg-v2-no-co2-2ki61qpw-10year"
JOB_GROUP="2ki61qpw-10year-raw-output"
# Result dataset of the 2ki61qpw training job (beaker experiment 01KVTWB66Y1RZR3Q2CFT6P44VA).
...
launch_job () {
    ...
    gantry run \
        --name $JOB_NAME \
        --task-name $JOB_NAME \
        --description 'Raw-output 10-year ACE inference re-run (2ki61qpw)' \
...
for YEAR in 2015 1995; do
    ...
    launch_job "$JOB_NAME" "$SCRIPT_PATH/ace-inference-4deg-v2-no-co2-10year-${YEAR}-raw.yaml"
done
