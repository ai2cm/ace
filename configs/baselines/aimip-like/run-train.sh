...
# =============================================================================
# Noise-pathway expressivity arms (research task
# 2026-07-06-test-noise-pathway-expressivity-against-the-1-step-train-side-
# under-dispersion). Each is a single-knob perturbation of the v2 residual
# baseline (oshj5u79), truncated to 30 epochs, testing whether adding
# stochastic expressivity relieves the 1-step train-side under-dispersion:
#
#   1. nembed64      — conditioning-noise embedding dim 32 -> 64 (more
#                      capacity in the existing conditional-layer-norm noise
#                      pathway; no new pathway).
#   2. innoise-iso32 — 32 isotropic noise channels concatenated to the network
#                      input (new second pathway), conditioning noise unchanged.
#   3. innoise-gauss32 — same as arm 2 but the input-noise channels are
#                      gaussian rather than isotropic.
# =============================================================================

# --- arm 1: noise_embed_dim 64, seed 0 (1 GPU; Jupiter+Titan, high) ---
run_training "train-4deg-daily-v2-era5-only-nembed64.yaml" "train-4deg-daily-v2-era5-only-nembed64-rs0" 1

# --- arm 2: 32 isotropic input-noise channels, seed 0 (1 GPU) ---
run_training "train-4deg-daily-v2-era5-only-innoise-iso32.yaml" "train-4deg-daily-v2-era5-only-innoise-iso32-rs0" 1

# --- arm 3: 32 gaussian input-noise channels, seed 0 (1 GPU) ---
run_training "train-4deg-daily-v2-era5-only-innoise-gauss32.yaml" "train-4deg-daily-v2-era5-only-innoise-gauss32-rs0" 1
