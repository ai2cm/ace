...
for pert in p0k p2k p4k; do
  launch_job "ace2s-4deg-daily-era5-only-local-mlp-diagnostics-$pert"       "$SCRIPT_PATH/ace-inference-era5-$pert.yaml" "$CKPT_LOCAL_HEAD"
  launch_job "ace2s-4deg-daily-era5-only-ankur-local-mlp-diagnostics-$pert" "$SCRIPT_PATH/ace-inference-era5-$pert.yaml" "$CKPT_ANKUR_HEAD"
  launch_job "ace2s-4deg-daily-era5-only-local-mlp-$pert"                   "$SCRIPT_PATH/ace-inference-era5-$pert.yaml" "$CKPT_FULLY_LOCAL"
done
