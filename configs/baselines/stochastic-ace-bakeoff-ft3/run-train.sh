...
#
# 3-step fine-tunes of the daily bake-off residual ablation (wandb t91wu5rg),
# testing whether multi-step fine-tuning restores small-scale precipitation
# spectral power. Both arms take the loss on all 3 steps
# (optimize_last_step_only: false) and differ ONLY in whether the state is
# detached between steps:
#   detached  -- use_gradient_accumulation: true   (per-step backward, no BPTT)
#   bptt      -- use_gradient_accumulation: false  (one backward through all 3)
run_training "ft3-detached.yaml" "ace2s-bakeoff-arm1-nores-nogmr-ft3-detached-rs0" 8
run_training "ft3-bptt.yaml"     "ace2s-bakeoff-arm1-nores-nogmr-ft3-bptt-rs0"     8
