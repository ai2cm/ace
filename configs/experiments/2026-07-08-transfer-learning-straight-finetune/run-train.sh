...
run_training() {
  ...
  gantry run \
    --name "$job_name" \
    --description 'ACE straight fine-tune onto ERA5 (transfer-learning control pair)' \
...
# =============================================================================
# Straight-fine-tune-onto-ERA5 transfer-learning control pair.
#
# Each arm takes a 4°/daily ACE2S-SHiELD+ pre-train donor (epoch-120, end of
# the 1-step phase) and straight fine-tunes it onto ERA5 (4°/daily), warm-
# started via stepper_training.parameter_init.weights_path. It tests whether
# fine-tuning onto ERA5 preserves the donor's +4K uniform-SST and future-
# scenario responses or wipes them (the known 1°/6h failure).
#
# Both arms carry the ERA5-v2 4°/daily recipe (data + ERA5 stats + loaders +
# inline eval + optimizer, 1-step, 120 epochs, lr 1e-4), with the per-arm model
# block following each donor's architecture (parameter_init requires it):
#   - reproduction arm: paper (original-style) builder + per-variable loss
#     weights, no residual, no tnorm; donor = paper-arm epoch-120 checkpoint.
#   - modern arm: v2 builder + tnorm, residual, bare loss; donor = modern-arm
#     epoch-120 pre-train (wandb jnlquua6).
# Each config mounts its donor checkpoint via a "# arg: --dataset ..." header.
# research: investigations/2026-07-13-transfer-learning-straight-finetune,
#           tasks/2026-07-08-transfer-learning-straight-finetune-controls.
# 1 GPU each; batch_size 8 is global, matching the ERA5-only baselines.
# =============================================================================

# --- reproduction (paper-arch) arm, seed 0 (1 GPU) ---
run_training "train-4deg-daily-era5-finetune-reproduction.yaml" "train-4deg-daily-era5-finetune-reproduction-rs0" 1
