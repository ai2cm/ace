#!/bin/bash
# Evaluator runs that exercise the inline snow_season aggregator on the four
# masked-snow checkpoints, for comparison against the water-year traces in the
# snow-memory prototype report. The controls have no snow channel.
#
# Usage:
...
JOB_GROUP="ace2s-snow-season-inline-check"
...
run_evaluator() {
    ...
    local job_name="ace2s-snowseason-${arm}-inline-check"
    ...
    gantry run \
        --name $job_name \
        --task-name $job_name \
        --description "ACE2S snow-season inline check: $arm" \
