...
CONFIG_FILENAME="multi-step-fine-tune-config-only-eq-beaker.yaml"
...
declare -A PRE_TRAINED_WEIGHTS_DATASETS=( \
    [0]="01KNMWHQYRW5PAE9W9VZXPKN4T" \
)

for seed in 0
do
    job_name="ace-shield-multi-step-fine-tune-only-eq-rs${seed}"
...
# for seed in 0
# do
#     job_name="ace-shield-multi-step-fine-tune-energy-conserving-only-eq-rs${seed}"
#     # Offset seed for fine-tuning so that data shuffling is different than
#     # during pre-training, but still follows the same path for a given set
#     # random initialization weights.
#     fine_tune_seed=$((seed + SEED_OFFSET))
