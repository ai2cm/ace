...
#
# Output:
#   /climate-default/2026-06-25-temporal-diffusion/inference/video-pmd-spatiotemporal-25km-100km-global-5ch-ou-v2/test-2023-2024-ens4-region-lat-88to-44-lon0to360.zarr
#
...
#
# Run:  bash configs/experiments/2026-08-10-video-pmd-spatiotemporal-25km-100km-ou-v2-test-inference-latScap/run.sh
set -e

JOB_NAME="video-pmd-spatiotemporal-25km-100km-global-5ch-ou-v2-test-inference-latScap"
...
gantry run --allow-dirty \
    --name "$JOB_NAME" \
    --description 'Video PMD spatiotemporal v2 test-set inference, SOUTHERN polar cap, lat -88..-44, full longitude, 4-member ensemble, per-channel Ornstein-Uhlenbeck noise kernel, 5 channels, 25km/100km, LR-endpoints-in/HR-full-out. Checkpoint is epoch 41/200 (crashed, not resumed -- see yaml header). 2x GPU DDP on titan.' \
    --workspace "$WORKSPACE" \
    --priority urgent \
    --cluster "$CLUSTER" \
