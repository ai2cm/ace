...
gantry run --allow-dirty \
    --name "$JOB_NAME" \
    --description 'Video PMD spatiotemporal v2 test-set inference, SOUTHERN polar cap, lat -88..-44, full longitude, 4-member ensemble, per-channel Ornstein-Uhlenbeck noise kernel, 5 channels, 25km/100km, LR-endpoints-in/HR-full-out. Checkpoint is epoch 41/200 (crashed, not resumed -- see yaml header). 4x GPU DDP on titan.' \
    --workspace "$WORKSPACE" \
    --priority urgent \
    --cluster "$CLUSTER" \
