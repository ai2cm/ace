...
run_inference() {
  ...
  gantry run \
    --name "$job_name" \
    --task-name "$job_name" \
    --description "Coupled ERA5+UFS ensemble fine-tuning inference" \
...
for year in $(seq 2022 2023); do
    ...
    run_inference "./configs/ufs-era5-fully-coupled-v0/inference-config-coupled-ERA5-UFS-v0-out-of-sample-yr${year_str}.yaml" \
