...
CONFIG_FILENAME="${1:-run_configs/ace-inference-sst-config-4deg-era5-p2k.yaml}"
JOB_NAME="${2:-ace-inference-sst-4deg}"
JOB_GROUP="${3:-ace2-fm-sst-perts-2026-06-26}"
...
cd $REPO_ROOT && gantry run \
    --name $JOB_NAME \
    --task-name $JOB_NAME \
    --description 'Run ACE FM SST-perturbation inference' \
