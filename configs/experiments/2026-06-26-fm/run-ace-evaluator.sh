...
CONFIG_FILENAME="${1:-run_configs/ace-som-data-only-config-4deg-1xCO2-ic_0005.yaml}"
JOB_NAME="${2:-ace-som-data-only}"
JOB_GROUP="${3:-ace2-fm-som-2026-06-26}"
...
cd $REPO_ROOT && gantry run \
    --name $JOB_NAME \
    --task-name $JOB_NAME \
    --description 'Run ACE FM evaluator' \
