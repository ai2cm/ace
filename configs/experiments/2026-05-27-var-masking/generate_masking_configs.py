...
def _apply_common_settings(
    ...
    # Signal channel presence to the network via FiLM conditioning
    # (condition_on_channel_mask, SFNO builder) instead of concatenating
    # presence maps onto the input (include_channel_mask_inputs). The two are
    # mutually exclusive; the FiLM path conditions on the combined
    # data_mask & input_dropout_mask presence vector.
    step_cfg["builder"]["config"]["condition_on_channel_mask"] = True
