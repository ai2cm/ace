...
export FME_STATS_DIR=/pscratch/sd/e/elynnwu/fme-dataset/2026-06-02-E3SMv3-piControl-105yr-coupled-stats/uncoupled_atmosphere

FINETUNING_FILE=${FINETUNING_FILE:-finetuning.txt}
CONFIG_FILE=${CONFIG_FILE:-config-finetune.yaml}
...
if [[ ! -f "$FINETUNING_FILE" ]]; then
  echo "Missing finetuning file: $FINETUNING_FILE"
  exit 1
fi
...
print_job() {
  echo
  echo "Perlmutter fine-tuning job:"
  ...
  echo " - Pretrain job: ${PRETRAIN_JOB_ID:-(n/a)}"
...
while IFS= read -r FINETUNING || [[ -n "$FINETUNING" ]]; do
  [[ -z "$FINETUNING" ]] && continue
  [[ "$FINETUNING" =~ ^[[:space:]]*# ]] && continue
  [[ "$FINETUNING" == group\|tag\|* ]] && continue
  ...
  CKPT_TYPE=${CKPT_TYPE:-best_inference_ckpt}
  CKPT_PATH_OVERRIDE=${CKPT_PATH_OVERRIDE:-}
  RESUME_JOB_ID=${RESUME_JOB_ID:-}

  if [[ -n "$CKPT_PATH_OVERRIDE" ]]; then
    CKPT_PATH="$CKPT_PATH_OVERRIDE"
  elif [[ -n "$PRETRAIN_JOB_ID" ]]; then
    PRETRAIN_DIR="${PSCRATCH}/fme-output/${PRETRAIN_JOB_ID}"
    CKPT_PATH="${PRETRAIN_DIR}/training_checkpoints/${CKPT_TYPE}.tar"
  else
    echo "ERROR: Either ckpt_path or pretrain_job_id is required: $GROUP|$TAG"
    exit 1
  fi

  if [[ "$DRY_RUN" != "true" && ! -f "$CKPT_PATH" ]]; then
  ...
  if [[ -z "${RESUME_JOB_ID:-}" ]]; then
    ...
    sed -i "s|FME_CHECKPOINT_PATH|${CKPT_PATH}|" "$CONFIG_DIR/train-config.yaml"
  else
    cp "${PSCRATCH}/fme-output/${RESUME_JOB_ID}/job_config/train-config.yaml" "$CONFIG_DIR/train-config.yaml"
  fi

  cp "$0" "$CONFIG_DIR/run-finetune-perlmutter.sh"
  cp "$FINETUNING_FILE" "$CONFIG_DIR/finetuning.txt"
  ...
  {
    echo
    echo "${GROUP}|${TAG}|${SLURM_JOB_ID}|submitted|${ACCOUNT}|${QUEUE}|${CONSTRAINT}|${NODES}|${GPUS_PER_NODE}|${CPUS_PER_TASK}|${TIME_LIMIT}|${OVERRIDE_ARGS}|${PRETRAIN_JOB_ID}|${CKPT_TYPE}"
  } >> "$EXPERIMENTS_FILE"
done < "$FINETUNING_FILE"
