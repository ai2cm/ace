...
for A in $ARMS; do
  total=$((total+1))
  JOB="samudra-anchor4deg-${A}-coupled-scratch"
  out=$(gantry run --name "$JOB" --task-name "$JOB" \
    --description "4deg anchor arm ${A}: ocean trained from scratch inside the coupled system (frozen stochastic ACE2S atmosphere)" \
    ...
    --env WANDB_JOB_TYPE=training --env WANDB_RUN_GROUP=samudra-anchor-4deg-coupled-scratch \
    ...
    -- torchrun --nproc_per_node="${N_GPUS}" -m fme.coupled.train "${SCRIPT_PATH}/coupled/${A}-coupled-scratch.yaml" 2>&1)
