...
# 01KYQG3299VTW0MRPQ1EF312P5, itself resumed from the original killed run
# 01KYJF301F62EX8Y365B8E7JRJ):
#   01KYQG329F629C70X0QH2WKZPZ
#
# Output:
#   /climate-default/2026-06-25-temporal-diffusion/inference/video-pmd-spatiotemporal-25km-100km-global-5ch-ou-v1/test-2023-2024-ens4.zarr
#
...
#
# Run:  bash configs/experiments/2026-07-27-video-pmd-spatiotemporal-25km-100km-ou-test-inference/run.sh
set -e

JOB_NAME="video-pmd-spatiotemporal-25km-100km-global-5ch-ou-v1-test-inference"
...
gantry run \
    --name "$JOB_NAME" \
    --description 'Video PMD spatiotemporal test-set inference, GLOBAL (not patched), 4-member ensemble, OU per-channel noise, 5 channels, 25km/100km. 4x GPU DDP on titan.' \
    --workspace "$WORKSPACE" \
    --priority urgent \
    --cluster "$CLUSTER" \
