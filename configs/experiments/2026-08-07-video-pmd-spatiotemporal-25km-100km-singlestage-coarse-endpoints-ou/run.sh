...
# trainer, TRUE LR-endpoints-in/HR-full-out variant, OU per-channel
# noise-kernel variant, via gantry + torchrun DDP. Reads data from WEKA
# (climate-default). Identical setup to
# ../2026-08-07-video-pmd-spatiotemporal-25km-100km-singlestage-coarse-endpoints-flat/run.sh
# except the OU noise kernel (see video_train.yaml's header comment,
# including the endpoint-denoising verification) and a fresh JOB_NAME/
# description.
#
...
#
# Run:  bash configs/experiments/2026-08-07-video-pmd-spatiotemporal-25km-100km-singlestage-coarse-endpoints-ou/run.sh
set -e

JOB_NAME="video-pmd-spatiotemporal-25km-100km-global-5ch-singlestage-coarse-endpoints-ou"
...
gantry run --allow-dirty \
    --name "$JOB_NAME" \
    --description 'Spatiotemporal video PMD, SINGLE-STAGE, TRUE LR-endpoints-in/HR-full-out (endpoints_observed=false, coarse_endpoints_only=true -- coarse only at the two endpoint frames, nothing pinned, single joint network diffuses every frame). 5 channels, per-channel OU noise kernel (verified to correctly denoise the endpoint frames too), global, patch-trained. 4x GPU DDP on titan (weka). Fresh run.' \
    --workspace "$WORKSPACE" \
    --priority urgent \
    --cluster "$CLUSTER" \
