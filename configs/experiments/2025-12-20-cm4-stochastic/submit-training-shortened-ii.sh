...
CONFIG_FILENAME="ursa-stochastic-train-shortened-ii.yaml"
CONFIG_PATH=$SCRIPT_DIR/$CONFIG_FILENAME
CHECKPOINT="best_ckpt.tar"
export WANDB_RUN_GROUP=cm4_like_am4_random_co2_stochastic_sii_ec_train
for seed in 0 1
do
    weights_id=${weights_ids[$seed]}
    weights=${CHECKPOINT_ROOT}/${weights_id}/training_checkpoints/${CHECKPOINT}
    override="stepper.parameter_init.weights_path=$weights"
    wandb_name=cm4_like_am4_random_co2_stochastic_sii_ec-rs${seed}-train
...
declare -A weights_ids=( [0]="6552401" [1]="6552402" )
CONFIG_FILENAME="ursa-stochastic-train-limited-shortened-ii.yaml"
CONFIG_PATH=$SCRIPT_DIR/$CONFIG_FILENAME
CHECKPOINT="best_ckpt.tar"
export WANDB_RUN_GROUP=cm4_like_am4_random_co2_stochastic_limited_sii_ec_train
for seed in 0 1
do
    weights_id=${weights_ids[$seed]}
    weights=${CHECKPOINT_ROOT}/${weights_id}/training_checkpoints/${CHECKPOINT}
    override="stepper.parameter_init.weights_path=$weights"
    wandb_name=cm4_like_am4_random_co2_stochastic_limited_sii_ec-rs${seed}-train
    conda run --prefix $FME_VENV \
	  python -m fme.ace.validate_config --config_type train $CONFIG_PATH --override $override
    $TRAIN_SUBMISSION_SCRIPT \
        $FME_VENV \
