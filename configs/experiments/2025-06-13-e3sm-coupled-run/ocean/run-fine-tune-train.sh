...
EXISTING_RESULTS_OCEAN_DATASET=01K09B3Z2YQNESD2KZW8RT94FG
python -m fme.ace.validate_config --config_type train $CONFIG_PATH

N_RANDOM_SEED_RUNS=2

JOB_NAME="${JOB_STEM}-fine-tune-rs1"  # job name for the current random seed
...
EXPERIMENT_ID=$(
    gantry run $ALLOW_DIRTY \
        --name $JOB_NAME \
        --description "Saumdra E3SM fine training ${EXISTING_RESULTS_OCEAN_DATASET}" \
