...
JOB_NAME="eamv3-precip-loss-2x-eval"
JOB_GROUP="BK-E3SMv3-atmosphere-v0813-scale-precip-loss-2"
EXISTING_RESULTS_DATASET="01K4ZVKT6FVYB9RT3Y21ABAKM7"  # this contains the checkpoint to use for inference
CONFIG_FILENAME="ace-evaluator-config.yaml"
SCRIPT_PATH=$(git rev-parse --show-prefix)  # relative to the root of the repository
CONFIG_PATH="${SCRIPT_PATH}${CONFIG_FILENAME}"
 # since we use a service account API key for wandb, we use the beaker username to set the wandb username
BEAKER_USERNAME=$WANDB_USERNAME
