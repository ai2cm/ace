...
mask20-gmroff, mask0-gmron, mask0-gmroff (each co2default).

v3 additionally gets an sst axis (``sst_options_for_version``): ``sston`` pins
...
mask0, where it would be a no-op. v3 also gets a ``clock50`` arm (from
``TARGETED_ARMS``): at mask20, the GMR sentinel channel
(``__gmr_extra__surface_temperature``) is pulled into its own
...
the sst axis. v3 config names keep the co2default token. That grows v3 to
20 + 10 (mask20 sston, both gmr options) + 5 (gmron mask20 clock50) = 35
configs.
...
name. This is 2 (mask) x 2 (gmr) x 5 = 20 configs: gmron/gmroff x mask0,
gmron/gmroff x mask20.
"""
...
def sst_options_for_version(version: str) -> dict[str, float | None]:
    """SST_OPTIONS, restricted to the no-op (SST in uniform pool) except v3.

    v3 adds an ``sston`` ablation: ``surface_temperature`` is moved into its
...
# Used by v4's TARGETED_ARMS and, GMR-on only, by v3 (see iter_train_configs).
...
def iter_train_configs(
    ...
    if version == "v3":
        # v3 clock50 arm: on top of the uniform mask20 pool, pull the GMR
        ...
        # crossed with the sst axis. v3 has no co2 input, but keeps its
        # co2default name token.
