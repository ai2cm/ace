...
#
# Expect ~16.5h (ensemble_chunk_size=1, 4 GPUs, batch_size=8) -- same as the
# latest.ckpt run (../2026-07-22-video-pmd-5ch-flat-test-inference/).
#
# RETRY HISTORY (2026-09-03): OOM'd twice on titan at batch_size 8 then 4
# (176.97 -> 178.21 GiB of 178.36 GiB -- barely moved), which pointed away
# from batch size and toward a fixed, step-count-scaled cost: generate()'s
# single-stage path was missing @torch.no_grad() around its ~99-step EDM
# sampling loop, retaining a full backward graph through the whole UNet on
# every step. Fixed in fme/downscaling/video_models.py (commit b52c44f5f);
# batch_size restored to 8. PYTORCH_CUDA_ALLOC_CONF=expandable_segments kept
# as harmless extra headroom.
#
