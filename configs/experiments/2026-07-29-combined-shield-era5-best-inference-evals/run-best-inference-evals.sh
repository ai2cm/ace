...
# trainings, on each training's BEST-INFERENCE-SELECTED checkpoint.
#
# This wave re-runs the 2026-07-28 configs (byte-identical; the checkpoint is a
# gantry mount, not a config field) off `best_inference_ckpt.tar` instead of
# `ema_ckpt_0240.tar`. That file is the checkpoint the trainer itself selected:
# the epoch minimizing the weighted inference error, whose weight-1.0 entries in
# every one of the seven training configs are the three slab-ocean CO2-response
# evaluations (som_1xco2_5year, som_2xco2_5year, som_4xco2_5year). So selection
# is on climate-response fidelity, not on weather skill or the last epoch. All
# seven trainings set validate_using_ema: true, so best_inference_ckpt.tar holds
# EMA weights -- the same weight family as ema_ckpt_0240.tar, one epoch apart.
# Selected epochs: A 228, G 140, B 142, B' 146, C 152, D 144, E 206 (none 240).
#
...
#   ./run-best-inference-evals.sh                     # all 33 jobs
#   ./run-best-inference-evals.sh drop50-era5         # canary: one arm+mode
#   ./run-best-inference-evals.sh p4k trend46yr       # substring OR filter
...
JOB_GROUP="combined-shield-era5-best-inference-evals"
CKPT_FILE="best_inference_ckpt.tar"
...
launch () {   # launch <job_name> <ckpt_dataset> <entrypoint...>
  ...
  gantry run \
    --name "$JOB_NAME" \
    --task-name "$JOB_NAME" \
    --description 'ERA5-case response evals, combined SHiELD+ERA5 arms, best-inference checkpoint' \
    --env WANDB_NAME="$JOB_NAME" \
    "${gantry_common[@]}" \
    --dataset "$CKPT_DATASET":"training_checkpoints/$CKPT_FILE":/ckpt.tar \
    -- "$@"
}
