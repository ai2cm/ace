#!/bin/bash
# One-time rollouts that save daily fields for the snow-memory metric prototype.
#
# Usage:
#   ./run-ace-evaluator.sh              # submit all six
#   ./run-ace-evaluator.sh cm4          # optional substring filter on the job name
#
# The masked-naive arms also write the shared target for their dataset, so they
# are submitted first.

set -e

JOB_GROUP="ace2s-snow-memory-rollouts"
...
run_evaluator() {
    ...
    local job_name="ace2s-snowmem-${arm}-rollout"
    ...
    gantry run \
        --name $job_name \
        --task-name $job_name \
        --description "ACE2S snow-memory rollout: $arm" \
