#!/bin/bash
# Full GPU training run for the spatiotemporal video PMD trainer, v2
# (endpoint-only coarse conditioning), via gantry + torchrun DDP. Reads data
# from WEKA (climate-default), same as v1
# (../2026-07-24-video-pmd-spatiotemporal-25km-100km/run.sh).
#
...
#
# Cluster/workspace: ai2/titan + ai2/climate-titan, same as v1 -- see that
...
#
# Run:  bash configs/experiments/2026-08-05-video-pmd-spatiotemporal-25km-100km-flat-v2/run.sh
#
# Diagnostics-only env vars (carried over from v1's 2026-07-25 debugging):
# TORCH_NCCL_TRACE_BUFFER_SIZE / TORCH_DISTRIBUTED_DEBUG=DETAIL don't change
# what's computed, only add a small constant overhead per collective op, in
# exchange for a real stack trace if a collective ever desyncs again instead
# of a silent 30min hang.
set -e

JOB_NAME="video-pmd-spatiotemporal-25km-100km-global-5ch-flat-v2"
