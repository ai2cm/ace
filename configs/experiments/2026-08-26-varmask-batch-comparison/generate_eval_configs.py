"""Generate evaluator suite configs for the arm x seed training runs.
...
# Placeholder for runs whose Beaker result dataset does not exist yet; refresh
# the map with update_beaker_map.py once the training jobs have finished.
PENDING_DATASET_ID = "PENDING"
...
CHECKPOINT_NAME_SUFFIXES = ("-bestinf",)
...
def generate_eval_config(
    ...
    source_dataset_id = source_map.get(source_run_name, PENDING_DATASET_ID)
...
def main() -> None:
    ...
    source_configs = sorted(RUN_CONFIGS_DIR.glob(f"{CONFIG_PREFIX}*-seed*.yaml"))
