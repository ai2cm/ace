...
#
# Output:
#   /climate-default/2026-06-25-temporal-diffusion/inference/video-pmd-spatiotemporal-25km-100km-global-5ch-ou-v1/test-2023-2024-ens4-region-lat-88to-44-lon0to360.zarr
#
...
#
# Run:  bash configs/experiments/2026-08-03-video-pmd-spatiotemporal-25km-100km-ou-test-inference-latScap/run.sh
set -e

JOB_NAME="video-pmd-spatiotemporal-25km-100km-global-5ch-ou-v1-test-inference-latScap"
...
gantry run --allow-dirty \
    --name "$JOB_NAME" \
    --description 'Video PMD spatiotemporal test-set inference, region 4/4 (southern polar cap, lat -88..-44, lon 0..360), 4-member ensemble, OU per-channel noise, 5 channels, 25km/100km. 4x GPU DDP on titan.' \
    --workspace "$WORKSPACE" \
    --priority urgent \
    --cluster "$CLUSTER" \
