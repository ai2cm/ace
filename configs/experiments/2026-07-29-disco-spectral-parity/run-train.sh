...
# =============================================================================
# Two-branch global DISCO vs the SFNO spectral filter, 4deg/daily no-residual.
#
# Matched pair. Both arms are the v2 ERA5-only no-residual recipe with
# filter_num_groups: 1 and spectral_ratio dropped to 1.0, because the DISCO
# filter supports neither grouped channels nor the pre/post 1x1 channel
# projections -- so the only difference between the arms is the filter itself:
#
#   reference: filter_type: linear                (dhconv spectral filter)
#   disco:     local_blocks [0..7] + local_filter (two-branch DISCO analog)
#
# 8 GPUs, not 1. batch_size is global and divided by world size, so 8 GPUs put
# 1 sample (x n_ensemble 2) on each; the same global batch and the same seed
# make this the same optimization problem as a 1-GPU run, up to reduction
