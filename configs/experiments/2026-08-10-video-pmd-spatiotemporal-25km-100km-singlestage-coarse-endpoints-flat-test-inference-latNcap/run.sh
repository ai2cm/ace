...
#
# Output:
#   /climate-default/2026-06-25-temporal-diffusion/inference/video-pmd-spatiotemporal-25km-100km-global-5ch-singlestage-coarse-endpoints-flat/test-2023-2024-ens4-region-lat44to88-lon0to360.zarr
#
...
#
# Run:  bash configs/experiments/2026-08-10-video-pmd-spatiotemporal-25km-100km-singlestage-coarse-endpoints-flat-test-inference-latNcap/run.sh
set -e

JOB_NAME="video-pmd-spatiotemporal-25km-100km-global-5ch-singlestage-coarse-endpoints-flat-test-inference-latNcap"
...
gantry run --allow-dirty \
    --name "$JOB_NAME" \
    --description 'Video PMD spatiotemporal SINGLE-STAGE (true LR-endpoints-in/HR-full-out) test-set inference, NORTHERN polar cap, lat 44..88, full longitude, 4-member ensemble, flat/independent noise, 5 channels, 25km/100km. Checkpoint is preliminary/undertrained -- see yaml header. 2x GPU DDP on titan.' \
    --workspace "$WORKSPACE" \
    --priority urgent \
    --cluster "$CLUSTER" \
