...
# trainer, v2 (endpoint-only coarse conditioning), via gantry + torchrun
# DDP. Reads data from WEKA (climate-default), same as v1
# (../2026-07-31-video-pmd-spatiotemporal-25km-100km-single-stage/run.sh).
#
...
#
# Run:  bash configs/experiments/2026-08-05-video-pmd-spatiotemporal-25km-100km-singlestage-v2/run.sh
set -e

JOB_NAME="video-pmd-spatiotemporal-25km-100km-global-5ch-singlestage-flat-v2"
...
gantry run \
    --name "$JOB_NAME" \
    --description 'Spatiotemporal video PMD v2, SINGLE-STAGE (joint spatial+temporal, no endpoint-SR sub-model), 5 channels, flat/independent noise, global, patch-trained, endpoint-only coarse conditioning. 4x GPU DDP on titan (weka). Fresh run, not resumed.' \
    --workspace "$WORKSPACE" \
    --priority urgent \
    --cluster "$CLUSTER" \
