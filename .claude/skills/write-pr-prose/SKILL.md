...
1. Run `git fetch origin main`, then read the full diff from main (`git diff origin/main...HEAD`, or `gh pr diff <number>`), not just the recent commits, so the title and description reflect the change from main per the guidance in AGENTS.md.
