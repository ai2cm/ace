...
def save_ice_nd_netcdf(
    ...
    if "simass" in ds:
        sea_ice_ocean = xr.DataArray(
            1.5*905 + rng.uniform(0.0, 0.2, size=ds["simass"].shape),
            dims=ds["simass"].dims,
            coords=ds["simass"].coords,
        )
        if "mask_2d" in ds:
            sea_ice_ocean = sea_ice_ocean.where(ds["mask_2d"] > 0, float("nan"))
        ds["simass"] = sea_ice_ocean

    if "sisnmass" in ds:
        sea_ice_ocean = xr.DataArray(
            0.2*330 + rng.uniform(0.0, 0.05, size=ds["sisnmass"].shape),
            dims=ds["sisnmass"].dims,
            coords=ds["sisnmass"].coords,
        )
        if "mask_2d" in ds:
            sea_ice_ocean = sea_ice_ocean.where(ds["mask_2d"] > 0, float("nan"))
        ds["sisnmass"] = sea_ice_ocean

    if "LSRCi" in ds:
        sea_ice_ocean = xr.DataArray(
            rng.uniform(0.0, 0.2, size=ds["LSRCi"].shape),
            dims=ds["LSRCi"].dims,
            coords=ds["LSRCi"].coords,
        )
        if "mask_2d" in ds:
            sea_ice_ocean = sea_ice_ocean.where(ds["mask_2d"] > 0, float("nan"))
        ds["LSRCi"] = sea_ice_ocean

    if "LSNKi" in ds:
        sea_ice_ocean = xr.DataArray(
            1.5*905 + rng.uniform(0.0, 0.2, size=ds["LSNKi"].shape),
            dims=ds["LSNKi"].dims,
            coords=ds["LSNKi"].coords,
        )
        if "mask_2d" in ds:
            sea_ice_ocean = sea_ice_ocean.where(ds["mask_2d"] > 0, float("nan"))
        ds["LSNKi"] = sea_ice_ocean

    if "XPRTi" in ds:
        sea_ice_ocean = xr.DataArray(
            0.2*330 + rng.uniform(0.0, 0.05, size=ds["XPRTi"].shape),
            dims=ds["XPRTi"].dims,
            coords=ds["XPRTi"].coords,
        )
        if "mask_2d" in ds:
            sea_ice_ocean = sea_ice_ocean.where(ds["mask_2d"] > 0, float("nan"))
        ds["XPRTi"] = sea_ice_ocean

    if "LSRCs" in ds:
        sea_ice_ocean = xr.DataArray(
            rng.uniform(0.0, 0.2, size=ds["LSRCs"].shape),
            dims=ds["LSRCs"].dims,
            coords=ds["LSRCs"].coords,
        )
        if "mask_2d" in ds:
            sea_ice_ocean = sea_ice_ocean.where(ds["mask_2d"] > 0, float("nan"))
        ds["LSRCs"] = sea_ice_ocean

    if "LSNKs" in ds:
        sea_ice_ocean = xr.DataArray(
            1.5*905 + rng.uniform(0.0, 0.2, size=ds["LSNKs"].shape),
            dims=ds["LSNKs"].dims,
            coords=ds["LSNKs"].coords,
        )
        if "mask_2d" in ds:
            sea_ice_ocean = sea_ice_ocean.where(ds["mask_2d"] > 0, float("nan"))
        ds["LSNKs"] = sea_ice_ocean

    if "XPRTs" in ds:
        sea_ice_ocean = xr.DataArray(
            0.2*330 + rng.uniform(0.0, 0.05, size=ds["XPRTs"].shape),
            dims=ds["XPRTs"].dims,
            coords=ds["XPRTs"].coords,
        )
        if "mask_2d" in ds:
            sea_ice_ocean = sea_ice_ocean.where(ds["mask_2d"] > 0, float("nan"))
        ds["XPRTs"] = sea_ice_ocean
...
def _setup(
    ...
    monthly_dim_sizes = get_ice_dim_sizes(n_time=10 * 12, nz_levels=1)
    monthly_out_vars = [
        v for v in out_variable_names if not re.match(r".+_\d+", v)
    ]
...
def test_train_and_inference(tmp_path, very_fast_only: bool):
    ...
    for var in ["siconc", "simass", "sisnmass"]:
        assert (
    ...
    for name in ["siconc", "simass", "sisnmass"]:
