...
class DistributedRealFFT2(nn.Module):
    ...
    def __init__(self, nlat, nlon, lmax=None, mmax=None):
        ...
        # shapes
        self.lat_shapes = makani_dist.compute_split_shapes(self.nlat, self.comm_size_h)
        self.lon_shapes = makani_dist.compute_split_shapes(self.nlon, self.comm_size_w)
        self.l_shapes = makani_dist.compute_split_shapes(self.lmax, self.comm_size_h)
        self.m_shapes = makani_dist.compute_split_shapes(self.mmax, self.comm_size_w)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if self.comm_size_w > 1:
            chan_shapes = makani_dist.compute_split_shapes(num_chans, self.comm_size_w)
        ...
        if self.comm_size_h > 1:
            chan_shapes = makani_dist.compute_split_shapes(num_chans, self.comm_size_h)
...
class DistributedMatmul(nn.Module):
    ...
    def forward(self, x):
        x_cp = dist.copy_to_parallel_region(x, self.comm_out_name)
        x_loc = self.matmul_handle(x_cp, self.weight, bias=None)
        x_out = dist.reduce_from_parallel_region(x_loc, self.comm_inp_name)
...
class DistributedPatchEmbed(nn.Module):
    def __init__(
        ...
        # gather shapes
        self.gather_shapes = dist.compute_split_shapes(
            in_chans, comm.get_size("matmul")
        )

    def forward(self, x):
        if self.input_parallel:
            x = dist.gather_from_parallel_region(x, 1, self.gather_shapes, "matmul")

        if self.output_parallel:
            x = dist.copy_to_parallel_region(x, "matmul")
...
class DistributedAFNO2Dv2(nn.Module):
    def __init__(
        ...
        self.gather_shapes = dist.compute_split_shapes(
            self.num_blocks, matmul_comm_size
        )
