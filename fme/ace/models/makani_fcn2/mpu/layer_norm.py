...
def distributed_welford_variance(
    ...
    vars_means_counts = dist.gather_from_parallel_region(
...
class DistributedInstanceNorm2d(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        with amp.autocast(device_type="cuda", enabled=False):
            ...
            mean = dist.copy_to_parallel_region(mean, "spatial")
            var = dist.copy_to_parallel_region(var, "spatial")
...
class DistributedGeometricInstanceNormS2(DistributedInstanceNorm2d):
    ...
    def _stats_welford(self, x: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        var = torch.sum(
            torch.square(x - mean.reshape(B, C, 1, 1)) * self.quad_weight,
            dim=(-2, -1),
            keepdim=False,
        )
        count = torch.tile(
            torch.sum(self.quad_weight, dim=(-2, -1), keepdim=False), (B, C)
        )
