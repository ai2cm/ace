...
class CrossFormer(BaseModel):
    def __init__(
        ...
        """WXFormer base architecture.

        Uses cross-embed convolutions and long/short distance
        attention in the encoder, with strided transpose
        convolution blocks for the decoder.

        Args:
            image_height: Grid cells (south-north).
            patch_height: Grid cells per patch (south-north).
            image_width: Grid cells (west-east).
            patch_width: Grid cells per patch (west-east).
            frames: Number of input time steps.
            channels: Number of 3D variables.
            surface_channels: Number of surface variables.
            input_only_channels: Input-only variable count.
            output_only_channels: Output-only variable count.
            levels: Vertical levels per 3D variable.
            dim: Hidden dimensions per encoder stage.
            depth: Attention blocks per encoder stage.
            dim_head: Dimension of each attention head.
            global_window_size: Long-range attention windows.
            local_window_size: Short-range attention window.
            cross_embed_kernel_sizes: Kernel sizes per stage.
            cross_embed_strides: Stride per stage.
            attn_dropout: Attention dropout rate.
            ff_dropout: Feedforward dropout rate.
            use_spectral_norm: Apply spectral normalization.
            interp: Interpolate output to input size.
            padding_conf: Padding configuration dict.
            post_conf: Postblock processing config dict.
        """
        super().__init__()
        ...
        if self.use_post_block:
            ...
            if "skebs" in post_conf.keys():
                if post_conf["skebs"].get("activate", False) and post_conf["skebs"].get(
                    ...
                    # freeze base model weights for skebs
