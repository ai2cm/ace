...
from fme.ace.models.makani_mpu.layer_norm import (
...
class SphericalFourierNeuralOperatorNet(torch.nn.Module):
    ...
    def __init__(
        ...
        if (dist.comm_get_size("spatial") > 1) and (not thd.is_initialized()):
            polar_group = (
                None if (dist.comm_get_size("h") == 1) else dist.comm_get_group("h")
            )
            azimuth_group = (
                None if (dist.comm_get_size("w") == 1) else dist.comm_get_group("w")
            )
        ...
        if dist.comm_get_size("spatial") > 1:
            self.img_shape_loc = (
                self.trans_down.lat_shapes[dist.comm_get_rank("h")],
                self.trans_down.lon_shapes[dist.comm_get_rank("w")],
            )
            self.img_shape_eff = (
                self.itrans_up.lat_shapes[dist.comm_get_rank("h")],
                self.itrans_up.lon_shapes[dist.comm_get_rank("w")],
            )
...
SFNO = physicsnemo.Module.from_torch(
    SphericalFourierNeuralOperatorNet, SphericalFourierNeuralOperatorNetMetaData()
)
