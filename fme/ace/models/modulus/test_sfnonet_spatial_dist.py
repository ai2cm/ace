...
from fme.core.dataset.test_helper import (
    gather_helper_conv,
    init_seed,
    relative_error,
    split_helper_conv,
...
def test_sfnonet_spatial_dist_output_is_unchanged():
    ...
    h_parallel_size = 2
    w_parallel_size = 2
    dist._init_distributed(
        h_parallel_size=h_parallel_size, w_parallel_size=w_parallel_size
    )
    ...
    inp_local = split_helper_conv(
        inp_full_device, hdim=-2, wdim=-1, w_group=w_group, h_group=h_group
    )
    ...
    state_dict_gather_full = checkpoint_helpers.gather_model_state_dict(
        model_dist, grads=True
    )
