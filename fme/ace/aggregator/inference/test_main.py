...
def test_inference_evaluator_aggregator_channel_mean_names(
    ...
    target_batch_data = BatchData(data=target_data, time=time)
    gen_batch_data = BatchData(data=gen_data, time=time)

    target_ensemble_data = target_batch_data.broadcast_ensemble(n_ensemble=n_ensemble)
    gen_ensemble_data = gen_batch_data.broadcast_ensemble(n_ensemble=n_ensemble)

    paired_data = PairedData.from_batch_data(
        prediction=gen_ensemble_data, reference=target_ensemble_data
    )
