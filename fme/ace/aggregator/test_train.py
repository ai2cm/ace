    ...
    agg.record_batch(
        batch=TrainOutput(
            metrics={
                ...
                "loss/corrector_penalty": torch.tensor(0.25, device=device),
                "loss/corrector_penalty_step_0": torch.tensor(0.5, device=device),
            },
            target_data=EnsembleTensorDict(
                {"a": torch.randn(batch_size, 1, n_time, nx, ny, device=device)},
            ),
            gen_data=EnsembleTensorDict(
                {
                    "a": torch.randn(
                        batch_size, n_ensemble, n_time, nx, ny, device=device
                    )
                },
            ),
            time=xr.DataArray(np.zeros((batch_size, n_time)), dims=["sample", "time"]),
            ...
            per_channel_losses={
                "a": ChannelLossInfo(loss=torch.tensor(0.5, device=device), count=4),
                "corrector_penalty/a": ChannelLossInfo(
                    loss=torch.tensor(0.125, device=device), count=4
                ),
            },
        ),
    )
    logs = agg.get_logs(label="train")
    assert logs["train/mean/loss/corrector_penalty"] == 0.25
    assert logs["train/mean/loss/corrector_penalty_step_0"] == 0.5
    assert logs["train/mean/loss/corrector_penalty/a"] == 0.125
