    ...
    # assert len(logs) > 0
      ...
      for ax in figure.get_axes():
        ...
        for line in ax.get_lines():
          x_data = line.get_xdata()
          y_data = line.get_ydata()
          np.testing.assert_allclose(y_data_ref, y_data, rtol=5e-05, atol=1e-10, equal_nan=False, err_msg='', verbose=True)
          np.testing.assert_allclose(x_data_ref, x_data, rtol=1e-08, atol=1e-13, equal_nan=False, err_msg='', verbose=True)
