...
class NoiseConditionedModule(nn.Module):
    ...
    Generates noise (gaussian by default, or via a custom factory) and
    optional positional embeddings (with label-position interaction),
    ...
    Args:
        ...
        noise_factory: Optional callable (batch_size, device, dtype) -> Tensor
            that produces noise of shape [batch_size, embed_dim_noise, H, W].
            If None, gaussian noise is generated.
    """
    ...
    def _generate_noise(self, x: torch.Tensor) -> torch.Tensor:
        """Generate noise. Subclasses can override for custom noise types."""
        return torch.randn(
