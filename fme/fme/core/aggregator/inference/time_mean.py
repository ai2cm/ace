...
class TimeMeanScalarMetricAggregator:
    """Scalar statistics on the time-mean state.

    This aggregator is currently used to keep track of
    time-mean RMSE across all channels.
    In the future, we can extend it to keep track of other scalar metrics.
    """
    ...
    def get_dataset(self) -> xr.Dataset:
        ...
        for pred in preds:
            rmse_all_channels[pred.name] = pred.rmse(weights=self._area_weights)
        all_channel_rmse = sum(rmse_all_channels.values()) / len(rmse_all_channels)
        return xr.Dataset(
            data_vars={
                "rmse_all_channels": xr.DataArray(
                    all_channel_rmse, attrs={"units": "-"}
                )
            }
        )
