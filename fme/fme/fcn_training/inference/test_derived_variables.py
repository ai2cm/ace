...
def test_compute_derived_variable():
    fake_data = {"PRESsfc": torch.tensor([1.0])}
    data = SteppedData(
    ...
    derived_variable = DerivedVariableRegistryEntry(
        func=lambda data, _: 3.0 * data.surface_pressure
    )
...
def test_compute_derived_variable_raises_value_error_when_overwriting():
    ...
    with pytest.raises(ValueError):
        _compute_derived_variable(data, sigma_coordinates, "PRESsfc", derived_variable)
