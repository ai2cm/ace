...
def test_paired_patches_with_random_offset_consistent(overlap):
    ...
    topography = StaticInputs(
        [StaticInput(data=topography_data, coords=full_fine_coords[0])]
    )
    y_offsets = []
    x_offsets = []
...
def test_paired_patches_shuffle(shuffle):
    ...
    topography = StaticInputs([StaticInput(data=topography_data, coords=fine_coords)])
    ...
    topo_concat_0 = torch.concat([t0[0].data for t0 in topography0], dim=0)
    topo_concat_1 = torch.concat([t1[0].data for t1 in topography1], dim=0)
