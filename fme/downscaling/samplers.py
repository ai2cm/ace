...
def stochastic_sampler(
    ...
    # Use float32 on MPS (no float64 support), float64 elsewhere for precision
    working_dtype = torch.float32 if latents.device.type == "mps" else torch.float64

    # Time step discretization.
    step_indices = torch.arange(num_steps, dtype=working_dtype, device=latents.device)
    t_steps = (
        sigma_max ** (1 / rho)
        + step_indices
        / (num_steps - 1)
    ...
    # Main sampling loop.
    x_next = latents.to(working_dtype) * t_steps[0]
