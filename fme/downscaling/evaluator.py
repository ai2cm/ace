...
class Evaluator:
    def __init__(
        ...
        model: BasePredictor | PatchPredictor,
        experiment_dir: str,
        n_samples: int,
...
class EvaluatorConfig:
    ...
    def _build_event_evaluator(
        ...
        if (dataset.coarse_shape[0] > predictor.coarse_shape[0]) or (
            dataset.coarse_shape[1] > predictor.coarse_shape[1]
        ):
            evaluator_model = PatchPredictor(
                model=predictor,
                coarse_yx_patch_extent=predictor.coarse_shape,
