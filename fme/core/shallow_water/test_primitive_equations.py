...
class TestPrimitiveEquationsStepper:
    ...
    def test_coriolis_does_no_work(self):
        ...
        # Check that the Coriolis term does no work: V · (f×V) = 0
        # analytically. Verify this property directly.
    ...
    def test_passive_tracer_tracks_temperature_perturbation(self):
        ...
        assert diff < 1e-4, f"q should closely track T_pert: max diff = {diff:.3e}"
    ...
    def test_hydrostatic_geopotential_increases_upward(self):
        ...
        for k in range(1, N_LEVELS):
            assert (
                phi[:, k] > phi[:, k - 1]
            ).all(), f"geopotential should increase: level {k} not > level {k-1}"
    ...
    def test_geopotential_proportional_to_temperature(self):
        ...
        for k in range(1, N_LEVELS):
            assert (
                phi_warm[:, k] > phi_cold[:, k]
            ).all(), f"warm column should have higher geopotential at level {k}"
