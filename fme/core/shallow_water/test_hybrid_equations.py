...
def _sigma_stepper(**kw) -> HybridCoordinateStepper:
    ...
    return HybridCoordinateStepper(
        ...
        a_interface=a_int, b_interface=b_int,
        **kw,
    )
...
class TestHybridCoordinateStepper:
    ...
    def test_coefficient_validation(self):
        ...
        with pytest.raises(ValueError):
            HybridCoordinateStepper(
                shape=SHAPE,
                a_mid=a_mid[:-1], b_mid=b_mid,  # wrong length
                a_interface=a_int, b_interface=b_int,
            )
        with pytest.raises(ValueError):
            HybridCoordinateStepper(
                shape=SHAPE,
                a_mid=a_mid, b_mid=b_mid,
                a_interface=a_int[:-1], b_interface=b_int,  # wrong length
            )
    ...
    def test_level_pressures_sigma_case(self):
        ...
        assert (p_k - expected).abs().max() < 1.0, (
            "sigma case: p_k should equal b_k * P_SURFACE"
        )
    ...
    def test_level_pressures_hybrid_vary_with_ps(self):
        ...
        assert (dp_k - expected_dp_k).abs().max() < 1.0, (
            "p_k change should scale as b_k * Δp_s"
        )
        # Upper levels (small b) change less than lower levels
        assert (dp_k[:, -1] < dp_k[:, 0]).all(), (
            "upper levels (small b) should change less than lower levels"
        )
    ...
    def test_geopotential_increases_upward(self):
        ...
        for k in range(1, N_LEVELS):
            assert (phi[:, k] > phi[:, k - 1]).all(), (
                f"φ should increase: level {k} not > level {k-1}"
            )
    ...
    def test_geopotential_hybrid_vs_sigma(self):
        ...
        assert (phi_hyb - phi_sig).abs().max() < 1e-2, (
    ...
    def test_C_top_interface_is_zero(self):
        ...
        top = C_half[:, -1]
        rel_err = top.abs().max() / (dp_k.abs().max() * div.abs().max() + 1e-30)
        assert top.abs().max() < 1e-4, (
    ...
    def test_hybrid_sigma_matches_sigma_stepper(self):
        ...
        sig_stepper = SigmaCoordinateStepper(
            shape=SHAPE, n_levels=N_LEVELS, sigma_levels=sigma_levels,
            omega=0.0, diffusion_coeff=None,
        )
    ...
    def test_cam_coefficients_smoke(self):
        ...
        for k in range(1, N_LEVELS):
            assert (p_k[:, k] < p_k[:, k - 1]).all(), (
                f"pressure should decrease upward: level {k} not < level {k-1}"
            )
