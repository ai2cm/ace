...
class SigmaCoordinateStepper(nn.Module):
    ...
    def _sigma_dot_interfaces(
        ...
        for k in range(K):
            sigma_dot_half[:, k + 1] = (
                sigma_dot_half[:, k]
                - (d_ln_ps_dt[:, 0] + div[:, k] + v_dot_glps[:, k])
                * self.delta_sigma[k]
            )
    ...
    def compute_tendencies(
        ...
        # Same mean-subtraction for ln(p_s): the large constant part
        # (~11.5 for p_s ~ 100 kPa) would otherwise create O(|ln p_s|·ε)
        # error in the discrete gradient.
        log_ps = torch.log(p_s)  # (B, H, W)
        log_ps = log_ps - log_ps.mean(dim=(-2, -1), keepdim=True)
        ...
        vert_adv_uv = torch.stack(
            [-sigma_dot * dV_dsig, -sigma_dot * dV_dsig_v], dim=-1
        )
        ...
        ps_term = (kappa * T / p_s.unsqueeze(1)) * (
            dp_s_dt.unsqueeze(1) + v_dot_grad_ps
        )
