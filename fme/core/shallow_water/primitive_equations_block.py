...
class PrimitiveEquationsBlockStepper(nn.Module):
    ...
    def compute_tendencies(
        ...
        # Mean-subtract T before passing to the block: the W_sv weights
        # compute ∇φ from T via the hydrostatic relation, and subtracting
        # the spatial mean eliminates O(|T|·ε) discretization error from
        # grad(const), where ε is the discrete gradient's constant-field error.
        T_anom = T - T.mean(dim=(-2, -1), keepdim=True)
        zeros = torch.zeros_like(T)
        per_level = torch.stack([T_anom, ke, zeros, zeros], dim=2)  # (B, K, 4, H, W)
