...
def _random_state(n_levels=N_LEVELS, seed=0):
    ...
    T = torch.full((B, n_levels, *SHAPE), T_BACKGROUND) + 10.0 * bump.unsqueeze(
        0
    ).unsqueeze(0)
...
class TestPrimitiveEquationsBlockStepper:
    ...
    def test_resting_uniform_state_zero_momentum_tendency(self):
        ...
        assert (
            duv.abs().max() < 1e-5
        ), f"expected zero momentum tendency, got {duv.abs().max():.2e}"
    ...
    def test_momentum_tendency_matches_reference(self):
        ...
        assert rel_err < 5e-3, (
            f"momentum tendency mismatch vs reference: "
    ...
    def test_full_tendency_matches_reference(self):
        ...
        Both implementations use the same discrete operators (W_vs2 for
        T/q advection via adv_conv), so the tight 5e-3 tolerance
