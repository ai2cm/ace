...
class TestDiskMetricLogger:
    ...
    def test_log_flushes_each_line(self, log_dir):
        logger = DiskMetricLogger(log_dir)
    ...
    def test_resume_skips_steps_at_or_below_high_water_mark(self, log_dir):
        logger = DiskMetricLogger(log_dir)
        logger.log({"loss": 0.5}, step=0)
    ...
    def test_resume_catches_up_then_continues(self, log_dir):
    ...
    def test_non_scalar_values_are_skipped(self, log_dir):
        logger = DiskMetricLogger(log_dir)
    ...
    def test_all_non_scalar_skips_entire_line(self, log_dir):
    ...
    def test_empty_data_skips_line(self, log_dir):
        logger = DiskMetricLogger(log_dir)
        logger.log({}, step=0)
        logger.close()
    ...
    def test_string_values_are_logged(self, log_dir):
        logger = DiskMetricLogger(log_dir)
    ...
    def test_bool_values_are_logged(self, log_dir):
        logger = DiskMetricLogger(log_dir)
    ...
    def test_no_existing_file(self, log_dir):
    ...
    def test_corrupt_line_is_skipped(self, log_dir):
    ...
    def test_read_metrics_empty_directory(self, tmp_path):
        """read_metrics returns empty list when no file exists."""
        assert read_metrics(str(tmp_path)) == []

    def test_multiple_logs_same_step(self, log_dir):
