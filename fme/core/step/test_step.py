...
def test_build_channel_presence_vector_and_combines_input_dropout():
    # A channel present in data_mask but dropped by the input-dropout mask
    # yields a 0.0 column; a channel absent in data_mask stays 0.0 regardless.
    # The dropout mask is broadcast over the batch (leading dim 1), matching
    # what VariableMasking.sample_mask produces.
    data_mask = {
    ...
    # a: present but dropped -> 0; b: absent -> 0
    expected = torch.tensor([[0.0, 0.0], [0.0, 0.0]])
    torch.testing.assert_close(presence, expected)


def _get_channel_mask_conditioned_selector(
...
def _capture_channel_mask(step: SingleModuleStep) -> dict[str, torch.Tensor | None]:
    """Capture the Context.channel_mask the conditional model is called with.

    torch_module is the distributed DummyWrapper around the
    NoiseConditionedModel; conditional_model is called as ``(x, context)``.
    """
...
def test_step_condition_on_channel_mask_not_doubled_and_passes_mask():
    ...
    captured = _capture_channel_mask(step)
...
def test_step_condition_on_channel_mask_with_include_composes():
    ...
    """
    config = _get_channel_mask_conditioned_selector(include_channel_mask_inputs=True)
    img_shape = DEFAULT_IMG_SHAPE
    n_samples = 3
    step = get_step(config, img_shape)
    assert isinstance(step, SingleModuleStep)
...
def test_step_condition_on_channel_mask_allows_input_dropout():
    ...
    """
    config = _get_channel_mask_conditioned_selector(
        input_dropout=VariableMaskingConfig(default=BernoulliMaskingConfig(rate=0.5)),
    )
    # Should not raise.
    get_step(config, DEFAULT_IMG_SHAPE)


def test_step_condition_on_channel_mask_films_on_input_dropout():
    """Dropout -> FiLM end to end.

    With condition_on_channel_mask=True and input_dropout configured, a drawn
    mask that drops a known channel must make that channel's
    Context.channel_mask column 0.0 while data_mask-present columns stay 1.0.
    Proves FiLM conditions on the combined (data_mask & input-dropout)
    presence, the new combination this stacks on.
    """
    config = _get_channel_mask_conditioned_selector(
        input_dropout=VariableMaskingConfig(default=BernoulliMaskingConfig(rate=0.5)),
    )
    ...
    # Drop forcing_rad only; the mask is broadcast over the batch and lists
    # dropped channels only, as VariableMasking.sample_mask produces.
    _inject_input_dropout_mask(
        step,
        {"forcing_rad": torch.tensor([False], device=fme.get_device())},
    )
    step.step(
        args=StepArgs(
            input=input_data,
