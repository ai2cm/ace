...
def _build_channel_mask_dict(
    ...
    Args:
        ...
        input_dropout_mask: Per-channel synthetic dropout presence masks of
            shape ``[*leading]`` keyed by packed channel name, or None.
    """
    ...
    for name in in_names:
        ...
        if data_mask is not None and lookup_name in data_mask:
            ...
            real = torch.ones(*leading, device=device, dtype=torch.bool)
        ...
        if input_dropout_mask is not None and name in input_dropout_mask:
            ...
            combined = real
        mask_nd = combined.to(dtype=torch.float)
        result[name] = mask_nd.view(*leading, 1, 1).expand(*leading, *spatial)
    return result
