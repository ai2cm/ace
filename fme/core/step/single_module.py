...
class SingleModuleStepConfig(StepConfigABC):
    ...
    def __post_init__(self):
        ...
        if self.residual_prediction is not None:
            ...
            if self.residual_prediction.normalized:
                ...
                if self.normalization.grouped is not None:
                    # The residual transform divides by the pooled field stds,
                    # while a grouped step denormalizes with each group's stds,
                    # so a unit network output would not be one tendency std.
                    raise ValueError(
    ...
    def get_step(
        ...
        grouped_normalizer = self.normalization.get_grouped_network_normalizer(
            normalize_names, n_spatial_dims=dataset_info.n_spatial_dims
        )
