    ...
    def step(
        ...
        for suffix, multiplier in self.forcing_multipliers.items():
            ...
            for name in self.output_names:
                ...
                predictions[new_name] = step_output.output[name]

        # Multi-call produces only diagnostic outputs, never corrected ones, so
        # the uncorrected shadow is always empty here.
