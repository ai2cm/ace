...
class _CapturingAggregator(InferenceAggregatorABC):
    def __init__(self, name: str, summary: dict[str, float], loss: float | None):
        self.name = name
    ...
    def get_summary(self):
        return InferenceSummary(logs=dict(self._summary), loss=self._loss)
...
def _make_task(
    ...
    if loss is None and summary:
        # Default the checkpoint-selection loss to the sole summary value so the
        # weighted-error assertions read naturally from the summary dicts.
        loss = next(iter(summary.values()))
    data = SimpleInferenceData(
        initial_condition=_FakePS(samples=[0]),
        loader=[_FakeFD(samples=[i + 1]) for i in range(n_windows)],
    )
    aggregator = _CapturingAggregator(name, summary, loss)
    return (
        InferenceTask(
            name=name,
            data=data,
