...
class UniformMaskingConfig:
    ...
    Parameters:
        ...
        same_mask_per_batch: If ``True``, all members of a batch share the same
            masked variables; if ``False`` (default), each sample is masked
            independently.
...
class PerVariableMaskingConfig:
    ...
    def sample_mask(
        ...
        """
        if self.same_mask_per_batch:
            mask = _sample_per_variable(self, n_channels, 1, device)
            mask = _apply_co2_override(mask, self.co2_rate, channel_names, device)
            return mask.repeat(batch_size, 1)
