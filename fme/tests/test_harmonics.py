...
def test_constant_field(nlat, nlon, grid, constant):
    ...
    the first one.
    """
    constant_field = torch.tensor(constant, dtype=torch.float64).repeat(nlat, nlon)
    ...
    assert torch.all(torch.isclose(zero, coeffs[1:]))
...
def _roundtrip(field, grid):
    ...
    return isht(sht(field.type(torch.double)))
...
def test_roundtrip(nlat, nlon, seed, grid="legendre-gauss"):
    ...
    assert torch.all(torch.isclose(proj, _roundtrip(proj, grid)))
