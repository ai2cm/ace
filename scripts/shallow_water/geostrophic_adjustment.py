...
def plot_geopotential_and_wind(snapshots, lat, lon_n, save_days, outfile):
    ...
    for i, d in enumerate(days):
        ...
        ax.contour(lon_d, lat_d, spd, levels=5, colors="k", linewidths=0.7, alpha=0.7)
        sk = 4
        ax.quiver(
            lon_d[::sk],
            lat_d[::sk],
    ...
    fig.suptitle(
        "500 hPa geopotential anomaly Δφ₁ (colours)"
        " and wind V₁ (arrows + speed contours)\n"
...
def plot_vertical_structure(snapshots, lat, lon_n, day, outfile):
    ...
    fig.suptitle(
        ...
        "Anticyclone intensifies with height"
        " (hydrostatic coupling: φ_k depends on T"
        " at all lower levels)",
        fontsize=12,
    )
    fig.savefig(outfile, dpi=110)
...
def main():
    ...
    print(
        f"(may be underestimated at 32×64 resolution with ~{sigma_m/1e3:.0f} km dome)\n"
    )
