#!/bin/bash
# Runs d_block_characterization.py as a non-interactive batch job on Beaker
# (CPU only, weka mounted), waits for it to finish, and fetches the results
# (stdout log + JSON) into ./d_block_characterization_results/.
#
# Same launch pattern as run_crps_eval.sh / run_endpoint_vs_interior_diagnostic.sh
# (see run_crps_eval.sh's header) -- submits, polls for completion, exits.
#
# Prereqs: beaker CLI installed and authenticated (`beaker account whoami`).
#
# Run:  bash scripts/video_pmd_eval/run_d_block_characterization.sh
set -e

SESSION_NAME="run-d-block-characterization-$(date +%s)"
...
RESULT_DIR="./d_block_characterization_results"
POLL_INTERVAL=15  # seconds

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
PY_SCRIPT="$SCRIPT_DIR/d_block_characterization.py"
...
CREATE_OUTPUT=$(beaker session create \
    ...
    --name "$SESSION_NAME" \
    --result /results \
    -- bash -c "set -e; echo $B64 | base64 -d > /tmp/d_block_characterization.py; cd /results; python3 -u /tmp/d_block_characterization.py 2>&1 | tee /results/output.log; echo D_BLOCK_CHAR_DONE" 2>&1)
...
if [ -n "$RESULT_DATASET" ]; then
    ...
    echo "Results saved to $RESULT_DIR/ (output.log, d_block_results.json)"
else
