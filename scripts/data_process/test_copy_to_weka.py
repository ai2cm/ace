...
def _config(include_time_coarsen: bool = True, trailing_slash: bool = False) -> Config:
    config_data: dict = {
        "runs": {"run-a": "", "run-b": ""},
...
def _write_config(tmp_path, **kwargs) -> str:
    ...
    config_data: dict = {
        ...
        "stats": {
            ...
            "data_type": config.stats.data_type,
        },
    }
    if config.time_coarsen is not None:
        config_data["time_coarsen"] = {
            "data_output_directory": config.time_coarsen.data_output_directory,
            "stats_output_directory": config.time_coarsen.stats_output_directory,
        }
...
def test_copy_pairs_rejects_destination_outside_weka_mount():
    with pytest.raises(click.ClickException, match="must be a path under"):
        _copy_pairs([RAW_DIRECTORY + "/run-a.zarr"], "/net/nfs/target")


def test_copy_pairs_rejects_weka_mount_root():
    with pytest.raises(click.ClickException, match="must be a path under"):
        _copy_pairs([RAW_DIRECTORY + "/run-a.zarr"], "/climate-default")
