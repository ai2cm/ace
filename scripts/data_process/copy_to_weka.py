...
def _sources_from_config(
    ...
    if coarsened and not config.has_time_coarsen:
        raise click.ClickException(
            "--coarsened was given but the config has no time_coarsen section."
        )
    available = config.run_names()
    selected = list(runs) if runs else available
    unknown = [run for run in selected if run not in available]
    if unknown:
        raise click.ClickException(
            f"Runs {unknown} are not in the config, which defines {available}."
        )
    if not selected:
        raise click.ClickException("The config defines no runs to copy.")
    if coarsened:
...
def _copy_pairs(sources: list[str], destination: str) -> list[CopyPair]:
    ...
    if root != WEKA_MOUNT and not root.startswith(WEKA_MOUNT + "/"):
        raise click.ClickException(
            f"Destination {destination} must be {WEKA_MOUNT} or a path under it, "
            "which is the only WEKA directory this job mounts."
        )
    pairs = []
    for source in sources:
        ...
        if not name or name == "gs:":
            raise click.ClickException(
                f"Could not determine a store name from source {source}."
            )
        pairs.append(CopyPair(source=source.rstrip("/"), destination=root + "/" + name))
    return pairs


def _exists(store: str) -> bool | None:
    """Whether a store exists, or None if that could not be determined.

    Returns None rather than raising when e.g. no GCS credentials are available
    locally, so that a credentials problem does not block submitting the job.
    """
    # The GCS client logs its own traceback when credentials are stale, which would
    # drown out the warning below for what is only a best effort check.
    logging.disable(logging.ERROR)
    try:
        filesystem, _, (path,) = fsspec.get_fs_token_paths(store)
        return filesystem.exists(path)
    except Exception as err:
        reason = err
    finally:
        logging.disable(logging.NOTSET)
    logging.warning(f"Could not check whether {store} exists: {reason}")
    return None


def _missing_sources(pairs: list[CopyPair]) -> list[str]:
    """Sources that were checked and do not exist."""
    return [pair.source for pair in pairs if _exists(pair.source) is False]
